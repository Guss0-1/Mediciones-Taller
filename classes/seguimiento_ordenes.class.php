...
class seguimiento_ordenes { 
	 ...
	 protected $_ci = Array('value' => '', 'datatype' => DBType::String, 'validators' => array()); 
	 ...
	 /** 
	  * Crea una nueva instacia del objeto seguimiento_ordenes. Inicializa las propiedades del objeto.
	  */ 
	 ...
	 public function set_ci($p_ci){ 
		 if(!DataValidator::validate($p_ci, $this->_ci['validators'])) 
			 throw new Exception('Error al establecer el valor de <strong>ci</strong>:<br/>' . DataValidator::get_error_text()); 
		 $this->_ci['value'] = $p_ci; 
	 } 
	 public function get_ci(){ return $this->_ci['value']; } 
	 ...
	 public function carga($p_id) { 
		 $query = new DBQuery('SELECT * FROM seguimiento_ordenes WHERE id = {id}'); 
	 ...
	 public function guarda() { 
		 if($this->_id['value'] == null) { 
			 $query = new DBQuery('INSERT INTO seguimiento_ordenes(ot,fecha_ingreso, vehiculo_modelo, cliente, ci, chasis, chapa, nro_asesor,email_asesor, fecha_fin_estado_terminado, id_estado_actual)VALUES({ot},{fecha_ingreso}, {vehiculo_modelo}, {cliente}, {ci}, {chasis}, {chapa}, {nro_asesor}, {email_asesor}, {fecha_fin_estado_terminado}, {id_estado_actual})'); 
			 ...
			 $query->addParam('ci', $this->_ci['value'], $this->_ci['datatype']); 
			 ...
			 $query = new DBQuery('UPDATE seguimiento_ordenes SET fecha_ingreso = {fecha_ingreso},ot = {ot}, vehiculo_modelo = {vehiculo_modelo}, cliente = {cliente}, ci = {ci}, chasis = {chasis}, chapa = {chapa}, nro_asesor = {nro_asesor}, email_asesor = {email_asesor}, fecha_fin_estado_terminado = {fecha_fin_estado_terminado}, id_estado_actual = {id_estado_actual}  WHERE id = {id}'); 
	 ...
	 public function elimina() { 
		 $query = new DBQuery('DELETE FROM seguimiento_ordenes WHERE id = {id}'); 
		 $query->addParam('id', $this->_id['value'], $this->_id['datatype']); 
		 $filas_afectadas = $this->_dbmanager->executeNonQuery($query); 
	 ...
	 public static function lista($p_opciones) { 
		 ...
		 $sql = 'SELECT  seguimiento_ordenes.*  FROM ' . $sql_from . ' ' . $filtro . ' ' . $orden; 
		 if($paginar === true){ 
	 ...
	 protected static function recupera_filtro_global($p_valor, $p_filtro = '') { 
		 ...
		 if(preg_match('/^\d+$/', $p_valor)) { 
			 $filtro.= ($filtro == '')?' ': ' OR '; 
			 $filtro.= "seguimiento_ordenes.id = " . DBManager::formatSQLValue($p_valor,"Integer") . " "; 
		 } 
		 if(preg_match('/(?:0[1-9]|[12][0-9]|3[01])\/(?:0[1-9]|1[0-2])\/(?:19|20\d{2})/', $p_valor)) { 
			 $filtro.= ($filtro == '')?' ': ' OR '; 
			 $filtro.= "seguimiento_ordenes.fecha_ingreso = " . DBManager::formatSQLValue($p_valor, "Date") . " "; 
		 } 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.vehiculo_modelo) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.ot) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.cliente) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.chasis) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.chapa) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 
		 
		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.nro_asesor) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") "; 

		 $filtro.= ($filtro == '')?' ': ' OR '; 
		 $filtro.= "LOWER(seguimiento_ordenes.email_asesor) LIKE LOWER(" . DBManager::formatSQLValue('%'.$p_valor.'%') . ") ";
