	  ...
	  )
	{
	    ...
	    $cuerpo = str_replace('%link_ot%', $seguimiento[0]['ot'], $cuerpo);
			...
			if($id_estado == 14 && $seguimiento[0]['id_estado_actual'] != 14){
				...
				if($asistencia_tecnica->guarda())
				{
					...
					$cuerpoAT = "<p>Se ha creado la asistenia tecnica numero $at : $link </p>";
	    ...
	    if(isset($_POST['preparacion_servicio']) || isset($_POST['preparacion_presupuesto'])){
					...
					if(isset($_POST['preparacion_servicio'])){
						...
						if (substr( $seguimiento[0]['ot'], 0, 3 ) === "OTH") {
    						$mail->AddCC('julio.orrego@perfecta.com.py', 'Julio Orrego');
						 	  ...
						 	  //$mail->AddCC('gustavo.galeano@perfecta.com.py', 'Gustavo Galeano');
    				}
					...
					if(isset($_POST['enviar_bps'])){
						//$mail->AddCC("emilio.cespedes@perfecta.com.py", "Emilio Cespedes");
					}
					//ASESOR DE SERVICIOS
					...
					if( $id_estado == 18 || $id_estado == 20){
						if (substr( $seguimiento[0]['ot'], 0, 3 ) === "OTH") {
							 ...
							 $mail->AddCC('katherine.noble@perfecta.com.py', 'Katherine Noble');
								...
								$mail->AddCC('torredecontrol.asu@perfecta.com.py', 'Torre de Control Asu');
						}
					}
